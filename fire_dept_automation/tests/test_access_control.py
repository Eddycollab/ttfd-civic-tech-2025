...
def test_authorized_user_can_upload(setup_test_db, test_image):
    ...
    print(
        f"✅ 成功：被分配的志工 '{setup_test_db['assigned_user']}' 成功上傳照片到 {photo_path}"
    )
...
def test_unauthorized_user_cannot_upload(setup_test_db, test_image):
    ...
    print(
        f"✅ 成功：未分配的志工 '{setup_test_db['unauthorized_user']}' 被正確阻止，錯誤訊息：{error_message}"
    )
...
def test_invalid_task_id(setup_test_db, test_image):
    ...
    with pytest.raises(ValueError) as exc_info:
        utils.save_proof_photo(
            test_image, invalid_task_id, current_user=setup_test_db["assigned_user"]
        )

    error_message = str(exc_info.value)
    assert "不存在" in error_message, "應該返回任務不存在的錯誤訊息"
    print(
        f"✅ 成功：無效的任務 ID {invalid_task_id} 被正確拒絕，錯誤訊息：{error_message}"
    )
...
def test_unassigned_task(setup_test_db, test_image):
    ...
    unassigned_task_id = db_manager.create_daily_task(
        "2024-01-02", setup_test_db["route_id"], None
    )

    with pytest.raises(PermissionError) as exc_info:
        utils.save_proof_photo(
            test_image, unassigned_task_id, current_user=setup_test_db["assigned_user"]
        )
