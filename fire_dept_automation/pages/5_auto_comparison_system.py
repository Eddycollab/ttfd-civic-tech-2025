...
if "user" in st.session_state and st.session_state.user:
    ...
    )  # 確保轉換為字典，避免 sqlite3.Row 沒有 get 方法的問題
    st.sidebar.success(
        f"👤 已登入：{current_user.get('username')} ({current_user.get('role')})"
    )
st.sidebar.divider()
...
def perform_ocr(image, tesseract_cmd):
    ...
    try:
        ...
        normalized_allowed = [
            os.path.normcase(os.path.normpath(p)) for p in ALLOWED_TESSERACT_PATHS
        ]
...
# Tesseract 路徑初始化
if (
    "tesseract_exe_path" not in st.session_state
    or not st.session_state["tesseract_exe_path"]
):
...
print("-" * 50, flush=True)
print(
    f"DEBUG: Check Tesseract Path: [{st.session_state.get('tesseract_exe_path')}]",
    flush=True,
)
print(
    f"DEBUG: Check Excel Source: [{st.session_state.get('system_excel_source')}]",
    flush=True,
)
print("-" * 50, flush=True)
# -----------------------------
...
with st.sidebar:
    ...
    with st.expander("2. 設定與資料來源", expanded=expand_config):
        ...
        uploaded_excel = st.file_uploader(
            "更換系統資料 (Excel)", type=["xls", "xlsx"], key="uploader_system_excel"
        )
...
with col1:
    ...
    with col_btn:
        force_reocr = st.button(
            "🔄 強制重新辨識", help="如果覺得辨識結果有誤，可點此重新執行 OCR"
        )
    ...
    with col_engine:
        ...
        use_fast_mode = st.checkbox(
            "⚡ 快速模式 (壓縮圖片)",
            value=True,
            help="降低圖片解析度 (150 DPI) 以加快 OCR 速度，但可能影響小字辨識率。",
        )
    ...
    with col_ai:
        ...
        use_vision_ai = st.checkbox(
            "啟用 Vision AI (實驗性)",
            value=False,
            help="使用多模態模型 (Llama 3.2 Vision) 直接分析圖片，可更準確識別目錄與表格結構，但速度較慢。",
        )
    ...
    if target_case and uploaded_file_path:
        if not os.path.exists(uploaded_file_path):
            ...
            if cache_miss or force_reocr or engine_changed:
                ...
                target_dpi = (
                    300  # 強制使用 300 DPI 以提升 OCR 對勾選框的辨識率 (User Request)
                )
                ...
                try:
                    ...
                    if ext == ".pdf":
                        # 顯示轉換訊息
                        with st.spinner(
                            f"📄 正在將 PDF 轉換為圖片 (DPI: {target_dpi})..."
                        ):
                            with open(uploaded_file_path, "rb") as f:
                                images = utils.pdf_to_images(f, dpi=target_dpi)
                    elif ext in [".doc", ".docx"]:
                        with st.spinner(
                            "📄 正在將 Word 文件轉換為 PDF (需安裝 Microsoft Word)..."
                        ):
                            temp_pdf_path = None
                            try:
                                temp_pdf_path = utils.convert_doc_to_pdf(
                                    uploaded_file_path
                                )
                        ...
                        if use_fast_mode and img.width > 1500:
                            ...
                            img = img.resize(
                                (1500, new_height), Image.Resampling.LANCZOS
                            )
                        images = [img]
                    else:
                        st.error(
                            f"❌ 不支援的檔案格式：{ext}。請上傳 PDF、Word 或圖片檔。"
                        )
                        images = []
                except Exception as e:
                    st.error(f"無法讀取檔案: {e}")
                ...
                if images:
                    ...
                    with st.spinner("🔍 正在進行 OCR 辨識中 (請稍候)..."):
                            ...
                            if use_paddle:
                                try:
                                    ...
                                    if "Error:" in ocr_text:
                                        st.warning(
                                            f"PaddleOCR 執行失敗 (第 {i + 1} 頁): {ocr_text}"
                                        )
                                    ...
                                    st.warning(
                                        f"PaddleOCR 執行失敗，切換至 Tesseract: {e}"
                                    )
                                    ocr_text = perform_ocr(img, tesseract_path)
                            else:
                                ocr_text = perform_ocr(img, tesseract_path)
                        ...
                        st.session_state.ocr_cache["pages_info"] = (
                            pages_info  # 儲存頁面資訊
                        )
            ...
            if use_ai_mode:
                ...
                if ai_engine.is_ollama_available():
                    ...
                    if cached_ai_result and cached_model == text_model:
                        ...
                        with st.spinner(f"🤖 AI ({text_model}) 正在分析文件內容..."):
                            if use_vision_ai:
                                # === Vision AI 混合模式 ===
                                st.info(
                                    "👁️ 正在使用 Vision AI 進行視覺化分析 (Llama 3.2 Vision)..."
                                )

                                # 1. 使用 Vision AI 分析文件結構與勾選項目 (針對圖片)
                                # 注意: 這裡假設使用者已安裝 llama3.2-vision
                                vision_result = ai_engine.analyze_document_structure(
                                    cached_images, model="llama3.2-vision"
                                )

                                # 2. 使用 Text AI 分析基本資料 (針對第一頁 OCR 文字)
                                # Vision 模型有時對密集文字的提取不如純文字模型穩定，因此混合使用
                                text_result = ai_engine.analyze_page_with_ai(
                                    page_one_text, model=text_model
                                )

                                # 3. 合併結果
                                ai_result = text_result
                                if vision_result.get("required_items"):
                                    ai_result["equipment_list"] = vision_result[
                                        "required_items"
                                    ]
                                    st.toast(
                                        f"Vision AI 成功提取 {len(ai_result['equipment_list'])} 項設備",
                                        icon="👁️",
                                    )
                                else:
                                    st.warning(
                                        "Vision AI 未能提取到設備清單，將使用 OCR 文字分析結果作為備案。"
                                    )

                            else:
                                # === 純文字模式 ===
                                ai_result = ai_engine.analyze_document(
                                    pages_text, model=text_model
                                )
                    ...
                    if "error" in ai_result:
                        ...
                        extracted_data = extract_info_from_ocr(
                            page_one_text, pages_text
                        )
                        # 應用簡繁轉換
                        ...
                        extracted_data = {
                            ...
                            "管理權人": clean_ai_value(
                                ai_result.get("management_person")
                            ),
                            "消防設備種類": process_equipment_list(
                                ai_result.get("equipment_list", [])
                            ),
                        }

                        # --- 強制獲取目錄頁碼 (TOC Page Number) ---
                        ocr_info_for_toc = extract_info_from_ocr(
                            page_one_text, pages_text
                        )
                        if "toc_page_num" in ocr_info_for_toc:
                            extracted_data["toc_page_num"] = ocr_info_for_toc[
                                "toc_page_num"
                            ]
                        ...
                        if not extracted_data.get("場所名稱"):
                            ...
                            fallback_data = extract_info_from_ocr(
                                page_one_text, pages_text
                            )
                        ...
                        with st.expander(
                            "🤖 查看 AI 完整分析結果 (JSON)", expanded=False
                        ):
                            st.json(ai_result)
                else:
            ...
            for i, img in enumerate(cached_images):
                ...
                with st.expander(
                    f"第 {i + 1} 頁 OCR 文字內容 (除錯用)", expanded=False
                ):
                    # 顯示每一頁的前30個字和完整內容
                    ...
                    if i < len(pages_text):
                        ...
                        preview_text = (
                            page_text[:30] if len(page_text) > 30 else page_text
                        )
...
with col2:
    ...
    with review_col1:
        ...
        applicant_email = st.text_input(
            "申請人信箱", value=default_email, placeholder="example@email.com"
        )
    with review_col2:
        st.write("審核結果通知：")
    ...
    # 取得 Email 設定
    sender_email = (
        st.secrets["email"].get("sender_email", "") if "email" in st.secrets else ""
    )
    sender_password = (
        st.secrets["email"].get("sender_password", "") if "email" in st.secrets else ""
    )
    ...
    if target_row is not None:
        ...
        if target_case and uploaded_file_path:
            ...
            st.caption(
                f"ℹ️ 已鎖定使用第 {toc_page_num} 頁內容進行自動填入 (消防設備種類)"
            )
        else:
        ...
        with st.expander("🔥 消防設備詳細比對與編輯", expanded=True):
            ...
            # 格式化 OCR 值 (顯示用)
            fmt_ocr_val = (
                st.session_state.modified_equip_ocr.replace("、", "\n")
                if st.session_state.modified_equip_ocr
                else ""
            )

            with col_equip1:
                st.text_area(
                    "系統列管設備 (唯讀)", value=fmt_sys_val, height=200, disabled=True
                )

            with col_equip2:
                new_equip_str = st.text_area(
                    "申報設備 (可編輯)",
                    value=fmt_ocr_val,
                    height=200,
                    help="若辨識有誤，請在此修正 (每行一項)",
                )
...
with tab_check:
    ...
    if "ocr_cache" in st.session_state and "pages_info" in st.session_state.ocr_cache:
        ...
        pages_info = st.session_state.ocr_cache.get(
            "pages_info", []
        )  # 在兩種模式都需要這個變數
        ...
        if use_vision_ai and images:
            ...
            try:
                ...
                if not ai_engine.is_ollama_available():
                    ...
                    if (
                        "vision_analysis" not in st.session_state
                        or st.session_state.get("vision_cache_key") != cache_key
                    ):
                        with st.spinner(
                            "🔍 Vision AI 正在分析文件結構 (可能需要 1-2 分鐘)..."
                        ):
                    ...
                    if result.get("error"):
                        ...
                        with col_v1:
                            ...
                            st.dataframe(
                                page_map_df, use_container_width=True, hide_index=True
                            )

                            if result["toc_page"]:
                                st.success(
                                    f"✅ 已識別目錄頁: 第 {result['toc_page']} 頁"
                                )
                                st.write("**目錄勾選項目:**")
                        ...
                        with col_v2:
                            ...
                            if (
                                ...
                                and not result["validation_report"].empty
                            ):
                                st.dataframe(
        ...
        if not use_vision_ai:
            with col_check_1:
                ...
                if toc_page:
                    ...
                    # Parse TOC (Lazy load)
                    if "detected_reqs" not in st.session_state or st.session_state.get(
                        "last_file_key"
                    ) != st.session_state.ocr_cache.get("file_key"):
                        with st.spinner("🔍 正在分析目錄勾選項目..."):
                            st.session_state.detected_reqs = (
                                doc_integrity.parse_toc_requirements(
                                    toc_img, toc_page["text"]
                                )
                            )
                    ...
                    selected_reqs = st.multiselect(
                        ...
                        options=all_docs,
                        default=[
                            d for d in st.session_state.detected_reqs if d in all_docs
                        ],
                    )
        ...
        with col_check_2:
            ...
            if not selected_reqs:
                ...
                for req in selected_reqs:
                    ...
                    report_data.append({"項目": req, "狀態": status, "備註": note})
