...
def render_task_management_ui(
    task_id, route_name, current_vol, event_date_str, username
):
    """
    ...
    with st.container():
        ...
        with col_actions[0]:
            if not current_vol:
                ...
                if st.button(
                    "🙋‍♂️ 我要認領", key=f"claim_tn_{task_id}", use_container_width=True
                ):
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🚚 今日配送", "🗓️ 排班與認領", "⚙️ 個案與路線管理", "📊 歷史紀錄與報表"]
    )
    ...
    with tab1:
        ...
        m3.metric(
            "✅ 已完成戶數",
            f"{completed_stops_count} 戶",
            delta=f"{completed_stops_count - total_stops_count} 待送"
            if total_stops_count > 0
            else None,
        )
        ...
        if not my_tasks:
            ...
            for task in my_tasks:
                ...
                if total_stops > 0:
                    ...
                    st.progress(
                        progress, text=f"配送進度：{completed_stops}/{total_stops}"
                    )
                ...
                for elderly in elderly_list:
                    ...
                    with st.expander(
                        f"{'✅' if is_delivered else '📦'} {name} - {address}",
                        expanded=not is_delivered,
                    ):
                        ...
                        with col1:
                            ...
                            st.link_button(
                                "🗺️ Google 導航", get_google_maps_url(address)
                            )
                            ...
                            if is_delivered:
                                ...
                                if photo is not None:
                                    ...
                                    with col_deliver:
                                        if st.button(
                                            ...
                                            try:
                                                ...
                                                db.create_delivery_record(
                                                    task_id,
                                                    elderly_id,
                                                    "已送達",
                                                    photo_path=photo_path,
                                                    volunteer_id=username,
                                                )
                                    ...
                                    with col_issue:
                                        if st.button(
                                            "⚠️ 異常",
                                            key=f"btn_err_{elderly_id}",
                                            use_container_width=True,
                                        ):
                                            st.session_state[
                                                f"show_issue_{elderly_id}"
                                            ] = True
                                else:
                                    st.warning("🚫 請先拍照才能送達")
                                ...
                                if st.session_state.get(f"show_issue_{elderly_id}"):
                                    ...
                                    issue_reason = st.selectbox(
                                        ...
                                        key=f"reason_{elderly_id}",
                                    )
                                    issue_note = st.text_area(
                                        "備註說明 (選填)", key=f"issue_{elderly_id}"
                                    )
                                    if st.button(
                                        "確認回報", key=f"confirm_issue_{elderly_id}"
                                    ):
                                        # 異常情況也必須有照片
                                        if photo is not None:
                                            try:
    ...
    with tab2:
        ...
        start_date = (datetime.date.today() - datetime.timedelta(days=30)).strftime(
            "%Y-%m-%d"
        )
        end_date = (datetime.date.today() + datetime.timedelta(days=60)).strftime(
            "%Y-%m-%d"
        )
        ...
        # 4. 顯示 Calendar
        cal_state = calendar(
            events=events, options=calendar_options, key="meal_calendar"
        )
        ...
        if cal_state.get("eventClick"):
            ...
            render_task_management_ui(
                task_id, route_name, current_vol, event["start"], username
            )
        ...
        with st.expander("➕ 新增排班任務"):
            with st.form("add_task_form"):
                ...
                new_task_route = st.selectbox(
                    "路線", options=routes_list, format_func=lambda x: x["route_name"]
                )
                if st.form_submit_button("新增任務"):
                    ...
                    exists = any(
                        t["route_id"] == new_task_route["id"] for t in existing
                    )

                    if exists:
                        st.error("該日期此路線已存在任務！")
                    else:
                        db.create_daily_task(
                            new_task_date.strftime("%Y-%m-%d"),
                            new_task_route["id"],
                            None,
                        )
                        st.toast("✅ 任務已建立！", icon="📅")
            ...
            with col_a:
                ...
                if profiles:
                    ...
                    column_config = {
                        ...
                        "address": "地址",
                        "phone": "電話",
                        "diet_type": st.column_config.SelectboxColumn(
                            "飲食類型", options=["一般", "素食", "切碎", "低鹽", "流質"]
                        ),
                        "route_id": st.column_config.SelectboxColumn(
                            "所屬路線",
                            options=[r["id"] for r in routes],
                            help="對應路線ID",
                        ),  # Ideally map to names, but ID is simpler for now or need a mapping
                    ...
                    if st.button("💾 儲存長者資料變更"):
                        ...
                        for index, row in edited_df.iterrows():
                            if pd.isna(
                                row["id"]
                            ):  # New row (ID is NaN usually for new rows in some configs, or we need to handle it)
                    ...
                    edited_df = st.data_editor(
                        df, num_rows="dynamic", key="elderly_editor_empty"
                    )
                    if st.button("💾 儲存新增資料"):
                        for index, row in edited_df.iterrows():
                            if row["name"]:
        ...
        if role != "admin":
            ...
            with c1:
                start_date = st.date_input(
                    "開始日期", datetime.date.today() - datetime.timedelta(days=30)
                )
            with c2:
            ...
            if start_date > end_date:
                ...
                # Fetch data
                report_data = db.get_delivery_reports(
                ...
                if report_data:
                    ...
                    column_config = {
                        "送達證明": st.column_config.ImageColumn(
                            "📸 送達證明", help="點擊查看大圖", width="small"
                        )
                    }

                    st.dataframe(
                        df_report, use_container_width=True, column_config=column_config
                    )
    ...
    if user_info and user_info["role"] == "admin":
        ...
        with st.expander("🔧 開發者除錯工具"):
            ...
            with col_debug1:
                ...
                if st.button(
                    "🗑️ 重置所有送餐資料", type="secondary", use_container_width=True
                ):
            ...
            with col_debug2:
                ...
                c.execute(
                    "SELECT COUNT(*) FROM daily_tasks WHERE date = ?",
