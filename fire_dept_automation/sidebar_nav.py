...
def render_chinese_sidebar():
    ...
    if st.sidebar.button(" 民眾申辦與查詢", use_container_width=True):
        st.switch_page("pages/3_public_application_and_inquiry.py")
    if st.sidebar.button(" 案件審核", use_container_width=True):
        st.switch_page("pages/4_case_review.py")
    if st.sidebar.button(" 申報書比對系統", use_container_width=True):
        st.switch_page("pages/5_auto_comparison_system.py")
    st.sidebar.divider()
