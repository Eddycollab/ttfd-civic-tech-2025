...
def save_login_session(username, role):
    ...
    st.session_state.pending_cookie_save = {"username": username, "role": role}
...
def process_pending_cookie_save():
    ...
    if "pending_cookie_save" in st.session_state:
        ...
        cookie_manager.set(
            "session_token", session_token, expires_at=expires_at, key="set_token"
        )
        cookie_manager.set(
            "username", data["username"], expires_at=expires_at, key="set_username"
        )
